...
import com.example.matthew.alarmclock.AlarmContract.Alarm;
...
public class AlarmDBHelper extends SQLiteOpenHelper {
	...
	private AlarmModel populateModel(Cursor c) {
        ...
        if(c.getInt(c.getColumnIndex(Alarm.COLUMN_NAME_ALARM_REPEAT_WEEKLY)) == 0)
        ...
        if(c.getInt(c.getColumnIndex(Alarm.COLUMN_NAME_ALARM_ENABLED)) == 0)
		...
		for (int i = 0; i < repeatingDays.length; ++i) {
            ...
            if(repeatingDays[i].equals("false"))
                model.setRepeatingDay(i, false);
            else
                model.setRepeatingDay(i, true);
		}
